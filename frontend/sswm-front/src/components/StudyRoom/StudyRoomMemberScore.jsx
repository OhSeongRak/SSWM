...
const StudyRoomMemberScore = ({ studyroomId }) => {
  ...
  useEffect(() => {
    const fetchTop3Attend = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BASE_URL}/api/studyrooms/${studyroomId}/daily-attend`,
          {
            headers: {
    ...
    const fetchTop3Study = async () => {
      // 출석률 top3
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BASE_URL}/api/studyrooms/${studyroomId}/daily-study`,
          {
            headers: {
              Authorization: accessToken,
            },
  ...
  return (
    <ContainerWrap>
      <ScoreWrap>
        {top3Study && (
          <ContentWrap>
            <ContentTitle>일일 공부왕</ContentTitle>
            <ContentRank>
                  ...
                  top3Study.map((v, idx) => (
                    <ContentRankValue key={idx}>
                      <MemberContent key={idx}>
                        <div style={{ marginBottom: "10px" }}>{formatTime(v.studyTime)}</div>
                        <Avatar
                          alt="Study-Member"
                          src={`${process.env.REACT_APP_IMAGE_URL}/${v.userDto.image}`}
                          sx={{ height: 70, width: 70 }}
                        />
                        <div style={{ display: "flex", textAlign: "center" }}>
                          <Nickname>{v.userDto.nickname}</Nickname>
                        </div>
                      </MemberContent>
                    </ContentRankValue>
                  ))}
        ...
        {top3Attend && (
          <ContentWrap>
            <ContentTitle>{top3Attend.month}월 출석왕</ContentTitle>
            <ContentRank>
              <ContentRankValueWrap>
                {top3Attend.users &&
                  top3Attend.users.map((user, idx) => (
                    <ContentRankValue key={idx}>
                      <MemberContent key={idx}>
                        ...
                        <Avatar
                          alt="Study-Member"
                          src={`${process.env.REACT_APP_IMAGE_URL}/${user.userDto.image}`}
                          sx={{ height: 70, width: 70 }}
                        />
                        <div style={{ display: "flex", textAlign: "center" }}>
                          <Nickname>{user.userDto.nickname}</Nickname>
                        </div>
                      </MemberContent>
                    </ContentRankValue>
                  ))}
...
const ContentTitle = styled.div`
  ...
  font-size: 25px;
  font-weight: bold;
  border-bottom: 3px solid #a4dbe4;
  margin-top : 30px;
`;
