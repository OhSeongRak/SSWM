...
const StudyRoomMemberScore = ({studyroomId}) => {
  ...
  useEffect(() => {
    const fetchTop3Attend = async () => {
      try {
        const response = await axios.get(`{REACT_APP_BASE_URL}/api/studyrooms/${studyroomId}/daily-attend`, {
          headers: {
    ...
    const fetchTop3Study = async () => {// 출석률 top3
      try {
        const response = await axios.get(`{REACT_APP_BASE_URL}/api/studyrooms/${studyroomId}/daily-study`, {
          headers: {
            Authorization: accessToken,
          },
        });
