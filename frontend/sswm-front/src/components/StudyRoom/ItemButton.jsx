...
const CardHoverButton = (props) => {
  ...
  const handleLeaveClick = () => {
    ...
    if (confirmLeave) {
      axios
        .put(
          `${process.env.REACT_APP_BASE_URL}/api/studyrooms/${studyroom.id}/leave`,
          {},
          {
            headers: {
  ...
  const handleButtonClick = async () => {
    ...
    if (studyroom.public === false) {
      ...
      const isEnterCodeMatch = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/studyrooms/enterCode`,
        {
          headers: {
            Authorization: accessToken,
          },
      ...
      if (isEnterCodeMatch.data === true) {
        const message = await axios.post(
          `${process.env.REACT_APP_BASE_URL}/api/studyrooms/${studyroom.id}/join`,
          {},
          {
            headers: {
              Authorization: accessToken,
        ...
        // 정원 초과라면 가입 불가
        if (message.data === "정원 초과입니다.") alert(message.data);
        else if (message.data === "가입 불가") alert("사용자가 차단되었습니다.");
        else if (message.data === "스터디룸 초과입니다.")
          alert("가입할 수 있는 스터디룸 개수를 초과하였습니다.");
        // 가입
        else window.location.href = `/StudyRoomMember/${studyroom.id}`;
      } else {
        // 입장 코드가 일치하지 않음
      ...
      // 정원 초과라면 가입 불가
      if (message.data === "정원 초과입니다.") alert(message.data);
      else if (message.data === "가입 불가") alert("가입이 불가능합니다.");
  ...
  return (
    <div>
      ...
      {studyroom.public === false &&
        props.isHost === false && ( //비공개 and 방장x
          <TextField
            sx={{
              margin: "30px",
              alignItems: "center",
